...
try{
    ...
    $actual=@(Get-ChildItem -LiteralPath $here -File -Recurse -Force|Where-Object{$_.Name -ne 'SHA256SUMS'})
    ...
    [ordered]@{schema_version='env-1b3-probe-error-v1';status='blocked';code=$code;exit_code=2}|ConvertTo-Json -Compress
    exit 2
}

...
try {
    ...
    Write-ENV1B3CaseResult -EvidenceRoot $EvidenceRoot -CaseId 'W06' -Result 'PASS' -Code 'ENV1B3_PERMISSION_MATRIX_PASS' -Evidence @{app_root_write_denied=$true; external_root_count=$externalRoots.Count; denied_root_checked=($Mode -eq 'DeniedWritableRoot')} | ConvertTo-Json -Depth 6 -Compress
} catch {

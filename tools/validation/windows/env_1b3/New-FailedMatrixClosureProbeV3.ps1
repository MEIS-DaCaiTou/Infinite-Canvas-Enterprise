...
try{
    ...
    $name='ENV-1B3-FAILED-MATRIX-CLOSURE-PROBE-V3-'+$head;$root=Join-Path $OutputRoot $name;$zip=$root+'.zip'
    ...
    [IO.File]::Copy((Join-Path $PSScriptRoot 'Invoke-FailedMatrixClosureProbeV3.ps1'),(Join-Path $root 'Invoke-FailedMatrixClosureProbeV3.ps1'),$false)
    ...
    $cases=@('W05','W08','W09','W10','W11StoppedPrepare','W11StoppedResume','W11RunningPrepare','W11RunningResume','W12EvidenceAudit','W13','ContractAudit')
    $doc=[ordered]@{schema_version='env-1b3-failed-matrix-closure-probe-v3';overall_task_id='ENV-1B3-CLEAN-WINDOWS-VALIDATION-AND-RELEASE-CANDIDATE';developer_head=$head;developer_tree=$tree;expected_candidate_id=$ExpectedCandidateId;expected_candidate_handoff_sha256=$ExpectedCandidateHandoffSha256;matrix_contract_filename=$contractRel;matrix_contract_sha256=$contractSha;diagnostic_only=$true;not_a_release_candidate=$true;cannot_support_final_acceptance=$true;production_approved=$false;executable_cases=$cases}

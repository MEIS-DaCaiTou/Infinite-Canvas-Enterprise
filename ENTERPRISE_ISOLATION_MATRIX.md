...
3G 第一阶段企业安全隔离底座已基本完成。当前矩阵不再只作为待实现清单，也作为后续端到端回归、上游同步复核和协作权限设计的基线。

已完成的关键域包括上传资源、素材库业务对象、WebSocket 广播、异步任务历史、管理员权限开关和审计。普通用户之间的默认策略仍是 owner 隔离；管理员可代管但关键操作必须审计；未知 owner / unowned 数据对普通用户默认拒绝。
...
Angle / Enhance ModelScope 上传解耦和上游 2026.06.30 同步不属于本矩阵当前完成状态，后续单独任务处理。

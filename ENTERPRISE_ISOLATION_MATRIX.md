...
当前项目还不是完整企业协作平台。下一阶段若引入协作，必须在本矩阵之上新增明确的成员、共享、授权、撤销和审计模型，而不是把 owner 字段临时改造成共享权限。

架构上继续采用“上游主应用 + enterprise gateway + interceptors + enterprise DB 映射”。但 `enterprise/interceptors.py` 已承载大量策略，后续新增隔离策略应逐步模块化到 `enterprise/policies/`。

Angle / Enhance ModelScope 上传解耦已由 PR #53 完成；上游 `2026.07.6` 受控同步已由 PR #61 完成；U-2-F2 已由 PR #62 修复文生图 / Enhance 云端刷新后历史记录丢失。后续仍需通过 3G-8 将浏览器级回归脚本化。
...
| 素材库 | `assets/library`、`data/asset_library.json`、`/api/asset-library*` | 上游全局 JSON 与目录；企业层按 owner 过滤 | `user_asset_object_map` 与业务 owner 回溯 | library/category/item 三级 owner；共享须明确 `visibility=shared` | 3G-4B / PR #38 已完成主页面、经典画布面板、Smart Canvas 面板、管理操作和 target owner 校验。 |
| 本地上传素材与文件夹 | `assets/uploads`、`/api/local-assets*` | 上游全局文件树；企业层按 owner 过滤 | `user_resource_map` | path/item owner；文件夹 owner、父级 owner、继承规则 | 3G-4A / PR #34 已完成上传 owner、直链鉴权、local-assets 基础隔离和输入复用校验。 |
| 共享文件夹 | `data/shared_folders.json`、`/api/shared-folders*` | 全局 | 最小安全收紧 | 默认管理员管理；如启用共享，显式 ACL，不可隐式共享 | 普通用户默认不可用，不能导致素材库页面加载失败；后续共享 ACL 另行设计。 |
...
| 在线生图与本地功能历史 | `history.json`、`/api/history`、`/api/history/delete`、在线/Angle/ZImage/Klein/Enhance 等结果 | 全局单文件；企业层按 owner 过滤 | `user_history_map`，资源复用 `user_resource_map` | `history_item_id -> owner`，历史图片复用 resource owner | 3G-3 / PR #28 已完成隔离；U-2-F2 / PR #62 修复 zimage/enhance/klein 云端 type 一致性。 |
...
| WebSocket 事件 | `/ws/stats`，`new_image`、`canvas_updated`、队列/状态事件 | 上游广播经企业层代理过滤 | connection registry + owner/task owner 判断 | connection -> user；事件携带 canvas/resource/task 时按 owner fan-out | 3G-5 / PR #42 已完成 stats/pong 保留、raw sensitive event 拦截和安全合成事件。 |
...
| `/api/canvas-comfy-tasks` | POST/GET | Comfy task | 全部 | 仅自己/所属画布 | 401 | `user_task_map` / `user_canvas_task_map`；输出资源继续写 `user_resource_map` | 管理员代管审计 | 3G-6 已建立 owner 拦截基线，外部成功链路后续补验 |

...
更新时间：2026-07-20
最后一次代码事实核对基线：`main@a53885b026a6c2440acb0fbde72d6571ff6f7723`

当前 repository HEAD 以 GitHub `main` 为准；PR #80 和 PR #81 已合并，ENV-1B1A 已进入 `main`。ENV-1B2P 当前只在 Draft PR 分支实施，尚未进入 `main`。
...
当前阶段为 **ENV-1B2P：Windows Python Runtime 分层来源证据验证**。ENV-1B0 和 ENV-1B1A 已分别由 PR #80、PR #81 完成并合并；ENV-1B2P 验证器、测试和分层结论当前仅存在于 Draft PR 分支。正常上游功能同步在 ENV-1 期间冻结；紧急安全漏洞修复可以单独评估并受控引入。
...
2. ENV-1B2P：Python 核心、依赖层、archive provenance 分层证据；当前 Draft PR 实施中，core `true`、dependency `false`、archive `false`、`production_approved=false`。
...
以上第 0、1 项已进入 `main`，第 2 项只在当前 Draft PR 分支实现，第 3 项及以后均为未实施工作。ENV-1B1A 只关闭 static 自修改并形成写入审计；ENV-1B2P 只分类既有证据，均不等于完整 APP_ROOT 只读或正式 Runtime 已获批。Fresh Install Bootstrap、新生产部署、OPS-3B、Linux、PostgreSQL、Redis、durable queue、多实例、Windows Service、正式 Windows Runtime Release 和 Production Baseline 当前都不是已实现能力。
...
1. release builder 与 release validator 增强；ENV-1B1A 已提供确定性 static staging builder，ENV-1B2P 当前 Draft 提供分层 Runtime 证据验证，完整 Release builder / validator 仍属后续。

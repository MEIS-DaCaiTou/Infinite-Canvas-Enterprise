...
当前 repository HEAD 以 GitHub `main` 为准；PR #80、PR #81 和 PR #82 已合并。ENV-1B1B 的 PathRoots / current-release 实现当前只在 Draft PR 分支，尚未进入 `main`。
...
| ENV-1B1B | 当前 Draft PR | PathRoots、严格 pointer 和核心路径迁移；不实施 activation、B1C 或完整 immutable release。 |
...
当前阶段为 **ENV-1B1B：PathRoots、版本目录与 current-release 原语**。ENV-1B0、ENV-1B1A 和 ENV-1B2P 已分别由 PR #80、PR #81、PR #82 合并；ENV-1B1B 实现仅存在于当前 Draft PR 分支。正常上游功能同步在 ENV-1 期间冻结；紧急安全漏洞修复可以单独评估并受控引入。
...
3. ENV-1B1B：路径根、版本目录和 `current-release.json`；当前 Draft PR 实施中，不含 activation。
...
以上第 0、1、2 项已进入 `main`，第 3 项只在当前 Draft PR 分支实现，第 4 项及以后均为未实施工作。ENV-1B1A 只关闭 static 自修改并形成写入审计；ENV-1B2P 只分类既有证据；ENV-1B1B 不接线正式入口或 activation，三者均不等于完整 APP_ROOT 只读或正式 Runtime 已获批。Fresh Install Bootstrap、新生产部署、OPS-3B、Linux、PostgreSQL、Redis、durable queue、多实例、Windows Service、正式 Windows Runtime Release 和 Production Baseline 当前都不是已实现能力。

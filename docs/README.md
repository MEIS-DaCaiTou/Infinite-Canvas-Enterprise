...
当前 repository HEAD：以 GitHub `main` 分支为准；PR #80 已完成 ENV-1B0 / DOC-2 / DOC-2A，PR #81 已合并并完成 ENV-1B1A。ENV-1B2P 验证器、测试和分层证据结论当前只存在于 Draft PR 分支，尚未进入 `main`。
...
- ENV：ADR-ENV-001 至 ADR-ENV-005 已由 PR #80 冻结；ENV-1B1A 已由 PR #81 合并完成；ENV-1B2P 是当前 Draft PR 实施阶段，ENV-1B1B / ENV-1B1C / 完整 ENV-1B2 均未开始。
- OPS：[ops/](./ops/)

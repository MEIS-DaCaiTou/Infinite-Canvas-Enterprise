...
当前 repository HEAD：以 GitHub `main` 分支为准；PR #80 已合并并完成 ENV-1B0 / DOC-2 / DOC-2A。本分支中的 ENV-1B1A 实现仍在 Draft PR 范围，尚未进入 `main`。
...
| ENV-1B1A 实施与写入审计 | [ENV-1B1A APP_ROOT 写入审计与 Static 构建](./env/ENV-1B1A-APP-ROOT-WRITE-AUDIT-AND-STATIC-BUILD-2026-07.md) | 区分本 PR 已关闭的 static blocker 与后续未迁移写入 |
...
- ENV：ADR-ENV-001 至 ADR-ENV-005 已由 PR #80 冻结；ENV-1B1A 当前在 Draft PR 中实施，ENV-1B1B / ENV-1B1C / ENV-1B2P 均未开始。
- OPS：[ops/](./ops/)

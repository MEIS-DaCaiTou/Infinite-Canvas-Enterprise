...
C3 correction pass 固化 workflow overlay 的读写边界：`workflow_path_from_name()` 仍只作为读取
overlay helper，`upload_workflow()` 的写入目标改为 user-only helper；同名上传只在
`USER_WORKFLOW_ROOT/custom/...` 创建 override，不改写 `SHIPPED_WORKFLOW_DIR`。user override 存在时
`list_workflows()` 与 `get_workflow()` 均返回 `builtin=false`；删除 override 后 shipped workflow 再次
可见并返回 `builtin=true`。这不改变 API 路由、请求/返回 schema、workflow JSON 格式、copy-on-edit
语义或 shipped-only delete 拒绝语义。
...
引入跨进程锁。C3 使 writer 返回不可变 `CurrentReleaseWriteResult`，成功时
`pointer_replaced=true`，并以 `directory_sync_status=synced|unsupported` 暴露目录同步结果；
`unsupported` 不表示 durability verified。目录同步 errno 分类按平台和阶段判断，EACCES/EPERM 不再
无条件归入 unsupported；replace 后 reparse 或其它 unexpected failure 统一按 post-replace durability
failure 处理。
...
| W29 | OPS report JSON / write_json 通过集中 validator 锚定 `STAGING_ROOT/reports`，并拒绝 APP_ROOT / RELEASE_ROOT / reparse / overlap；development compatibility 仍保留 | STAGING | 不是 Manifest v2 或 apply/switch |
| W30 | OPS JSONL and update job log/workspace logging 通过集中 validator 锚定 `LOG_ROOT/ops` 或安全 workspace，拒绝 APP_ROOT / RELEASE_ROOT / reparse / overlap；development compatibility 仍保留 | LOG / STAGING | 不是 Manifest v2 或 apply/switch |
| W31 | OPS backup helpers 通过集中 validator 锚定 `BACKUP_ROOT`，并拒绝 APP_ROOT / RELEASE_ROOT / reparse / overlap；development compatibility 仍保留 | BACKUP | 不是 Manifest v2 或 apply/switch |
| W32 | online-update download 保持显式外部 workspace / staging 边界；development compatibility 仍保留 | STAGING | 不是 Manifest v2 或 apply/switch |
| W33 | release staging 保持显式外部 workspace / staging 边界；完整 Release staging/activation 不在本 PR | STAGING | 不是 Manifest v2 或 apply/switch |
...
| W29 | partially_migrated | OPS report JSON / write_json portable target 已具备外部 `STAGING_ROOT/reports` 锚定和 fail-closed 测试；development compatibility 仍保留旧相对路径语义，因此不提升为完整 migrated。 |
| W30 | partially_migrated | OPS JSONL and update job log/workspace logging 已具备外部 LOG / STAGING 边界测试；development compatibility 仍保留旧相对路径语义，因此不提升为完整 migrated。 |
| W31 | partially_migrated | OPS backup helpers 已具备外部 `BACKUP_ROOT` 锚定和 fail-closed 测试；development compatibility 仍保留旧相对路径语义，因此不提升为完整 migrated。 |
| W32 | partially_migrated | online-update download 保持显式外部 workspace；完整 Release staging/activation 不在本 PR。 |
| W33 | partially_migrated | release staging 仍未实现 Manifest v2 / activation；完整 Release staging/activation 不在本 PR。 |
...
| W41 | migrated_state_primitive | `atomic_write_current_release` 是独立 `STATE_ROOT` persistent-state primitive；包含 owned-temp cleanup、atomic replace、observable directory sync result 和 post-replace durability failure 分类；没有 activation call site。 |
...
因此，本 PR 若合并也只能说明路径契约和核心迁移进入仓库。C3 已由独立远程代码复核确认关闭
workflow shipped write boundary、默认 DB resolver 统一校验和 current-release write result 契约三项
最终代码缺口；当前无未关闭代码 Blocker，等待项目负责人决定是否标记 GitHub Ready。
`ENV_1B1B_initial_implementation_present=true`、
`ENV_1B1B_scoped_contract_acceptance_passed=true`、
`ENV_1B1B_completion_classification=partial`、
`ENV_1B1B_full_release_acceptance_passed=false`、`ready_to_mark_ready=true`、`Ready=false`、
`ready_for_merge=false`、`merge_recommended=pending_project_owner_approval`、
`formal_release_blocking_deferred_count=5`、`non_blocking_deferred_count=5`、
`full_APP_ROOT_immutable=false`、`formal_release_created=false`、`Release_activation_implemented=false`、
`ENV_1B1C_started=false`、`Manifest_v2_implemented=false`、
`Production_Baseline_approved=false`、`production_approved=false`。
...
DB 默认/显式 candidate 统一校验、DB 文件级/父目录/连接后 reparse fail-closed、current-release malformed/duplicate/residual `.new`、
临时所有权故障注入、replace 后目录同步 result/unsupported/unexpected failure 分支、两阶段解析、
workflow 同名上传 user-only override、shipped digest 不变和 list/get `builtin` 一致性、

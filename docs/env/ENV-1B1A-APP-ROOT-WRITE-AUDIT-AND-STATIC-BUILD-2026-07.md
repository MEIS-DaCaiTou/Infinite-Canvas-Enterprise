...
4. `enterprise.release.app_root_audit` 只接收 Git tracked 文件集，排除 test fixture 和浏览器静态页面后，对每个生产写入 site 记录相对文件、qualified symbol、operation 和规范化调用 SHA-256 fingerprint，再映射到 W01-W45。
...
- 写入 inventory、Git tracked fingerprint manifest、W01-W45 锚点和 fail-closed 漂移测试形成；这仍不是对未知写入绝对不存在的证明。

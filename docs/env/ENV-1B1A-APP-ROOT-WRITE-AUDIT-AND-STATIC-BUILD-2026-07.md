...
这不等于 APP_ROOT 已只读。历史审计识别出 40 个功能写入流；后续 C1 新增 W41，ENV-1B1C-B1 新增 W42，ENV-1B2A 新增 W43，OPS Release Manifest v2 新增 W44，ENV-1B3 新增 W45，UPDATE-MVP-1 Draft 新增 W46。只有 static 自修改和新 builder 边界在 ENV-1B1A 内关闭；后续阶段已逐步迁移 Runtime 与 Release 边界，但 Production Baseline 和生产执行仍未形成。
...
4. `enterprise.release.app_root_audit` 只接收 Git tracked 文件集，排除 test fixture 和浏览器静态页面后，对每个生产写入 site 记录相对文件、qualified symbol、operation 和规范化调用 SHA-256 fingerprint，再映射到 W01-W46。
...
原始 ENV-1B1A 扫描的 40 个功能流是历史基线。ENV-1B1C-B1 R3 staged Git tracked 输入为 `scanned=91`、`excluded=249`、`detected=299`、`mapped=299`，历史冻结 digest 为 `464b2eef086b6fea37daf810d3b9f0551de652763f23028df799f8affb81e1ab`。ENV-1B2A correction tracked 输入为 `scanned=97`、`excluded=266`、`detected=322`、`mapped=322`，冻结 digest 为 `b69cfffca2f40a4f6b39af5b92673efbfede91d8ac3740017fc28914a9c69c71`。OPS Release Manifest v2 correction tracked 输入为 `scanned=100`、`excluded=275`、`detected=368`、`mapped=368`，冻结 digest 为 `b237e92a27ff6da88f70a6e542743ef0a6a6228fff7d5f9a9e47430cdd78299b`。ENV-1B3 initial candidate 输入为 `scanned=111`、`excluded=280`、`detected=375`、`mapped=375`，历史 digest 为 `d4b45c6a7b4679ed213e1836e78882522a9dbe0b9ba00fabb77cc034ba0ecaad`。materialization-stabilization 输入为 `scanned=113`、`excluded=280`、`detected=374`、`mapped=374`，历史 digest 为 `8e1f01dc49f135da57af5ab424d92bdb65a0cbf40fa1eb45904702d215b18a85`。Failed Matrix Closure Probe v3 输入为 `scanned=121`、`excluded=286`、`detected=383`、`mapped=383`，历史冻结 digest 为 `46e39d7a83266b880eda2cbb5d94b9220c0872707761a5962384e8651a8de3e5`。最终收敛输入为 `scanned=131`、`excluded=290`、`detected=384`、`mapped=384`，parse failure、uncovered site、stale mapping、missing anchor 和 invalid flow 均为 0；冻结 site manifest SHA-256 为 `4d8d78573e7c6f1981b2e3939443ee0f5259099f27e3228299d2d880c80fdeb1`。C1 新增 W41，将 current-release persistent-state primitive 从 W24 分离；C3 的 workflow 同名上传修复仍归入既有 W12。B1 的 launch-context publish 保持 W26 runtime-control-state 语义，writable-root probe 与 portable preflight 的五根目录准备同属独立 W42。ENV-1B2A 的构建器只接受显式 caller-owned 外部 artifact root，其写入统一归入 W43。OPS Release Manifest v2 的 build/materialization 写入归入 W44，ENV-1B3 的 caller-owned candidate/test/evidence 写入归入 W45；这些都不是 APP_ROOT 正式运行写入。ENV-1B2P 的 `runtime_provenance._atomic_write_report` 仍归入 W40，不表示 APP_ROOT 路径已迁移。该静态分析是可重复漂移门禁，不能证明绝对不存在动态或未知写入。

Candidate 07 strict Runtime repair 输入为 `scanned=132`、`excluded=290`、`detected=387`、`mapped=387`，parse failure、uncovered site、stale mapping、missing anchor 和 invalid flow 均为 0；冻结 site manifest SHA-256 为 `c197d5f6ce697dd86ccc607ca6db26fc2785ae581b424f37c2eb296961ba2a1d`。W11 reboot-stale reconciliation 的 exclusive claim、state replace 与 matching lock cleanup 继续属于既有 W26 Runtime control-state flow，不新增 APP_ROOT 写入流。

UPDATE-MVP-1 Draft 输入为 `scanned=136`、`excluded=294`、`detected=405`、`mapped=405`，parse failure、uncovered site、stale mapping、missing anchor 和 invalid flow 均为 0；site manifest SHA-256 为 `e86368690a7e37276ae189f306a24c3ad765d318c7be72d2411ffeb66386f8d6`。W46 只覆盖外部 STAGING/STATE/RELEASE roots 中的受控更新 job、全新 Release materialization 和 pointer-last 切换；运行中及历史 APP_ROOT 均不原地改写。
...
| W30 | `append_jsonl`、update job log | portable log 锚定 `LOG_ROOT/ops`，workspace 锚定 `STAGING_ROOT/workspace`，development compatibility 仍保留；可为是 | admin / update | job log；部分保留；可能 | OPS runner/update service | `LOG_ROOT` / `STAGING_ROOT` | partially_migrated；ENV-1B1B | C2 增加外部根成功与 APP_ROOT/reparse/overlap 拒绝；redaction 测试存在 |
| W31 | OPS backup helpers | portable backup 锚定 `BACKUP_ROOT`，development compatibility 仍保留；可为是 | backup | SQLite/JSON/assets backup；是；是 | OPS backup execute | `BACKUP_ROOT` | partially_migrated；ENV-1B1B | C2 增加 backup root 外部锚定与 new-target fail-closed；完整 backup/restore rehearsal 仍未实施 |
...
| W45 | `tools/validation/windows/env_1b3/Invoke-ENV1B3Validation.ps1` 及其内部 case scripts | caller-owned 仓库外 candidate / handoff / test / evidence root；否 | ENV-1B3 development build 与独立 Windows test-host validation | immutable handoff、materialized test Release、case evidence、isolated tamper copies；否；否 | exact candidate identity、explicit roots、W01-W14 matrix | 仓库外开发证据根或独立测试主机 root | validation-only；不接入 activation，不访问生产 | safe path/reparse、closed inventory、owned-copy cleanup、stable case IDs、audit drift gate |
| W46 | `enterprise/ops/update/mvp.py`、`enterprise/update_api.py` | `STAGING_ROOT/update-mvp`、新的 `RELEASE_ROOT/<release_id>`、`STATE_ROOT/system-update-active.lock`；运行中 APP_ROOT 否 | 授权 Update Center prepare / execute / one-shot handoff | bounded job/evidence、已验证全新 Release、active-job reservation；pointer 仍由 W41 写入 | Manifest v2、现有 Runtime supervisor、current-release expected identity | `STAGING_ROOT` / `RELEASE_ROOT` / `STATE_ROOT` | Draft 中的同 Schema/无 migration 单跳 MVP；不是通用 OPS-3B、数据库升级或生产授权 | 全局开关 + super_admin/admin explicit override、password reconfirm、one-active lock、target health、自动 pointer rollback、APP_ROOT audit |
...
- 写入 inventory、Git tracked fingerprint manifest、W01-W46 锚点和 fail-closed 漂移测试形成；这仍不是对未知写入绝对不存在的证明。

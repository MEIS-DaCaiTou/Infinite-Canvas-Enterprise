...
| W46 | `enterprise/ops/update/mvp.py`、`enterprise/ops/update/recovery.py`、`enterprise/update_api.py` | `STAGING_ROOT/update-mvp`、新的 `RELEASE_ROOT/<release_id>`、`STATE_ROOT/system-update-active.lock`；运行中 APP_ROOT 否 | 授权 Update Center prepare / execute / one-shot handoff / 恢复核验 | bounded job/evidence、恢复核验记录、已验证全新 Release、active-job reservation；pointer 仍由 W41 写入 | Manifest v2、现有 Runtime supervisor、current-release expected identity | `STAGING_ROOT` / `RELEASE_ROOT` / `STATE_ROOT` | 更新与人工解除阻断；不是自动数据库修复或生产授权 | 全局开关 + super_admin explicit override、password reconfirm、one-active lock、target health、失败时持续阻断、APP_ROOT audit |

...
READY 在线请求还必须携带已由认证中间件验证的 principal `auth_version`。治理事务在取得写锁后将其与 `main.users.auth_version` 精确比较；缺失、布尔值、非法值或不匹配均以脱敏的 `401 STALE_AUTHENTICATION` 拒绝。这样即使 Token 在中间件验证后被密码修改或会话撤销，旧请求也不能完成一次额外治理写入。

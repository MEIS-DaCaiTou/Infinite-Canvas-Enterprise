...
READY 下旧公开 `enterprise.db.create_user()` 对普通用户和管理员创建均关闭，不会计算 password hash、插入用户或提交事务。在线普通用户创建的唯一正式入口是 actor-aware `create_ordinary_user()`；临时回归 fixture 使用 tests 目录中的显式 SQLite helper，不向生产代码增加 test-only、bypass 或 skip-governance 参数。
...
每个 `main.users` INSERT 或 UPDATE 均保存 cursor 并要求 `rowcount == 1`。随后服务在同一 connection、同一事务内直接回读原始 SQLite 字段，不使用 `normalize_user_record()` 作为完整性证明：创建确认 `id`、username、display name、password hash、`role=user`、raw `is_admin=0`、`auth_version=1` 和 raw `is_active=1`；密码、active、soft delete 与 profile 操作同时确认预期变更和所有不应变化的原始安全字段。raw `is_admin` 和 raw `is_active` 必须是精确整数 `0` 或 `1`，role=user 对应 `is_admin=0`，admin/super_admin 对应 `is_admin=1`；`is_active=2`、bool 宽松比较或 role/is_admin 不一致均 fail closed。业务写入后、success audit 前还会重新确认 actor 的原始 role、is_admin、is_active、auth_version、password hash 和角色更新时间字段未被 trigger 或其它同事务副作用改变。
...
`_hash_password()` 只在 READY actor 当前性、角色策略、目标策略和创建字段策略通过后运行。stale actor、ordinary user 越权、admin 对受保护目标越权、含 `is_admin` / `role` 提权字段的创建请求和旧密码错误的自助修改均不会提前执行 PBKDF2；本阶段接受合法操作在写事务内计算 hash，后续性能阶段再评估两阶段预检优化。
...
`POST /api/users` 对已知治理异常仍使用稳定错误映射。LEGACY 和 READY 的重复 username 均返回 `409 USERNAME_CONFLICT`；READY 在写事务内先检查 username，避免重名请求计算 password hash。其它 SQLite integrity conflict 不伪装成用户名冲突，未知内部异常统一返回固定 `500 USER_CREATE_FAILED`，不将异常字符串、SQL、路径、异常类型或 traceback 写入客户端响应。
...
- 原始字段对抗测试覆盖单独篡改 raw `is_admin`、将 raw `is_active` 改为 `2`，以及单独篡改 actor raw is_admin/is_active；这些状态不会被 role 推导或 bool 规范化掩盖。
- READY 下旧 `edb.create_user(..., False/True)` 均拒绝且不写入；临时 READY fixture 使用 test-only SQLite helper。未授权、stale、受保护目标和提权字段拒绝路径均不会调用 PBKDF2，合法 create/reset/self-password 各验证一次 hash 调用。

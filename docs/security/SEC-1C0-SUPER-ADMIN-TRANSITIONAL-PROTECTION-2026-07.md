...
在线敏感治理进入 `enterprise/security_user_governance.py`。服务在 `BEGIN IMMEDIATE` 后从 `main.users` 重读 actor 和 target，执行角色策略、业务变更与 auth-version 更新，并使用同一 connection 追加 mandatory audit。audit 成功后才 commit。
...
任何验证、SQL 或 mandatory audit 失败都会 rollback。不存在“先提交用户变更，再补写审计”的路径。
...
拒绝统一使用 `security.authorization.denied`：普通角色越权为 L2，涉及 super_admin 目标或 `role=super_admin` 请求为 L3。L3 使用固定服务端 reason `transitional_super_admin_protection`。
...
`_require_admin` 继续作为粗粒度入口初筛，但 READY 最终授权只使用 request principal 中的 actor ID；actor role、active 和 target 状态由服务在事务内重读。

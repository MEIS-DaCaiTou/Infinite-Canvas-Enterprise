...
OPS-0 / OPS-0A 已完成，生产环境只读盘点已文档化。OPS-1 已完成，生产备份、离线发布包、升级演练、回滚、数据库迁移与数据治理方案已沉淀。OPS-2A 已完成工具落地，但生产环境尚未升级，也尚未执行生产侧工具验证。

当前下一步不是生产升级，而是生产侧只读 / 非破坏性试运行验证：

1. `inventory`
2. `check-data`
3. `backup` dry-run

`backup --execute` 需要单独确认，不在本阶段文档同步 PR 中触发。`validate-release` / `prepare-upgrade` 需要离线 release 包、backup manifest、data-check report 等前置输入，不会自动执行生产升级。

由于已有早期版本上线生产，且生产环境与开发 / Codex 环境隔离，3G-8 暂时后置但不取消。Docker / 1Panel / 日志体系纳入 OPS 后续主线。

下一步优先级：

1. OPS-2A post-merge production-side dry-run validation：生产侧 `inventory`、`check-data`、`backup` dry-run。

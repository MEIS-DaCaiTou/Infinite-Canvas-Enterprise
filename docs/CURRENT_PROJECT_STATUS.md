...
- `last_verified_code_baseline`：`a53885b026a6c2440acb0fbde72d6571ff6f7723`。
...
- ENV-1B2P：当前 Draft PR 对仓库外既有 Runtime 证据实施标准库、显式路径、无网络的分层验证，并已关闭 Runtime manifest 自声明提升路径：实际安装闭包采用 30 个 lock 项加固定 `pip`/`setuptools`/`wheel` bootstrap allowlist 精确核对，dependency / archive future true-path 必须提供独立 attestation、pip-check report 和 archive build record。真实结果仍为 core `true`、dependency `false`、archive `false`、`production_approved=false`；合并前不得描述为已进入 `main` 或正式 Runtime 已获批。
...
| ENV-1B2P | 当前 Draft PR | 只读验证既有仓库外证据；core 已验证，dependency 缺少独立 rebuild attestation / pip-check report，archive 缺少 assembled archive / 独立 build record；manifest 布尔或哈希字符串自声明无提升权，固定 `production_approved=false`。 |
...
| ENV / 不可变 Release | ENV-1B0 和 ENV-1B1A 已合并。static 启动自修改已移除，确定性 builder 和 Git tracked site fingerprint 到 W01-W40 的漂移门禁已进入 `main`；静态分析不证明绝对不存在未知写入。ENV-1B2P 当前 Draft 只验证 Runtime 分层证据，结果为 core `true`、dependency `false`、archive `false`。`assets`、`output`、`data`、配置、startup migration、legacy update、bytecode 等仍未迁移，因此完整只读 APP_ROOT、正式不可变 Release、Production Baseline 和 production approval 均未形成。 |
...
3. ENV-1B2P：当前 Draft PR 实施 Runtime 分层证据验证；core `true`，dependency / archive `false`，`production_approved=false`。该结论在合并前只属于当前分支。
4. ENV-1B1B、ENV-1B1C、完整 ENV-1B2、Manifest v2 和 ENV-1B3：按路线完成路径根、Release-bound 入口、可重复 Runtime、自检和干净 Windows 验证；均未开始。
...
从 GitHub 最新 `main` 开始新任务；`a53885b026a6c2440acb0fbde72d6571ff6f7723` 是 PR #81 合并后的 ENV-1B2P 起始基线，`be5573a`、`396cccc` 和 `a095ce2e` 只作为历史核对基线。main 前进后先读取 `docs/README.md`、本文件、当前 ADR 和最近 PR。

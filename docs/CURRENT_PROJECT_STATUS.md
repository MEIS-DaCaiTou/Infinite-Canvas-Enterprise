...
SEC-1A 已完成 ADR 决策，由 PR #71 承载；该 PR 只定义超级管理员、Capability 和高风险操作治理方向，没有实施安全能力。后续 SEC-1B1 与 SEC-1F0 分别建立 role / auth_version、JWT 当前状态和最小强制安全审计底层；`super_admin`、Capability、Step-up Authentication、Operation Token 和完整安全审计仍未实现。详细决策见 [ADR SEC-1A](./decisions/ADR-SEC-1A-SUPER-ADMIN-CAPABILITY-GOVERNANCE-2026-07.md)。
...
| OPS-3A | 已完成，PR #77 | 可信发布源、严格 manifest、受限下载、Windows 安全 staging、证据绑定的非执行 online-update plan；合并不代表生产检查、下载、升级或服务操作已经执行。 |
| STAB-1 / OPS-L1 | Draft PR #78，继续复核 | 本地 `3001` upstream / `8000` gateway 独立 supervisor、持久化脱敏日志、原子 runtime state、完整进程 identity、generation-bound lifecycle ACK、优雅 child shutdown 和 Windows Job Object 基础。第二轮 fixture 覆盖已通过；开发设备隔离真实应用在临时 DB、runtime root、端口和 Python 副本中完成 start/restart/stop、host 退出、端口释放、幂等 stop、再次 start/stop 及单角色恢复。`enterprise.auth` 的 `jwt` 依赖已补入 `requirements.txt`。PR 仍保持 Draft，以上不代表生产服务已切换或 Windows Service 已安装。 |
...
| OPS 工具 | OPS-2A / OPS-2B / OPS-3A 已进入 main；STAB-1 / OPS-L1 Draft PR #78 提供本地 runtime supervisor、持久化日志和 instance-bound lifecycle ACK 基础。隔离 fixture 回归已覆盖第二轮修复；隔离开发设备 Smoke 已完成 restart 后 stop 完成确认、host 退出、端口释放和单角色恢复，且 `requirements.txt` 已声明 gateway 所需 PyJWT。上述能力不代表生产已升级、服务已切换，也不代表 apply-upgrade / restore / rollback、Docker / 1Panel 或 PostgreSQL 已实现。 |
...
1. 项目负责人审阅 SEC-1B2 runbook，并在独立、受控维护窗口内决定是否执行 production activation；不得自动执行、不得以代码合并替代执行。
2. 按独立 Issue / Draft PR 继续 SEC-1C Capability 门禁、SEC-1D Step-up、SEC-1E UI、SEC-1F 完整审计和 SEC-1U 升级安全收口。
...
5. 审阅 OPS-3A Draft PR #77；生产计划、下载、staging 或 apply 必须继续由独立阶段和人工维护窗口决定，不得由合并自动触发。
6. 审阅 STAB-1 / OPS-L1 Draft PR #78，并在独立生产维护窗口前完成 Windows runtime 生命周期复核；不得由仓库合并自动启动、停止或重启生产服务。
7. 逐域推进 ARCH-3 policy 模块化和 PERF-1 真流式代理 / 性能基线。
8. OPS-D1 Docker / 1Panel 单机生产化后置到安全、数据和恢复基础稳定之后。
9. PostgreSQL、Redis、MinIO / S3、多实例和多服务器属于长期 P3 目标。

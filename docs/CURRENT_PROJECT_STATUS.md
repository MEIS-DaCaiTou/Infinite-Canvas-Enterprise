...
ARCH-2A、SEC-1A、SEC-1B1 和 SEC-1F0 仓库实现已完成。SEC-1B1 与 SEC-1F0 均没有激活生产 migration；当前下一阶段是 SEC-1C0 super_admin 过渡保护。首次 production migration activation 和 super_admin bootstrap 仍属于 SEC-1B2，且必须后置到 SEC-1F0 与 SEC-1C0。

每个 P0 安全事项必须使用独立 Issue、独立分支和独立 Draft PR，不将全部 P0 项目打包到一个大 PR。

下一步顺序：

1. SEC-1C0：在首次 bootstrap 前实施 super_admin 过渡保护，阻止 admin 影响 super_admin、阻止自行提权并保证正常在线事务不把 active super_admin 降为零；不实现完整 Capability 矩阵。
2. SEC-1B2：仅在 SEC-1F0 和 SEC-1C0 可用后受控激活 migration 并实施本机首次 super_admin bootstrap，依次进入 `UNINITIALIZED`、`ACTIVE`。
3. 按独立 Issue / Draft PR 继续 SEC-1C Capability 门禁、SEC-1D Step-up、SEC-1E UI、SEC-1F 完整审计和 SEC-1U 升级安全收口。
4. 设计 DATA-1 数据一致性、schema version、migration history 和人工 owner reconciliation 基础。
5. 推进 backup restore rehearsal；在 restore / rollback 未完成演练前，不接入网页 apply-upgrade。
6. 推进 OBS-1 / OPS-L1 日志与可观测性基础。

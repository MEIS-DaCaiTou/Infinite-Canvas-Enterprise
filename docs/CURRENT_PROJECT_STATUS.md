...
| OPS 工具 | OPS-2A / OPS-2B / OPS-3A 已进入 main；STAB-1 / OPS-L1 Draft PR #78 提供本地 runtime supervisor、持久化日志和 instance-bound lifecycle ACK 基础。隔离 fixture 回归已覆盖第二轮修复；真实开发设备 Smoke 仍在 restart 后第二次 stop 的完成确认处阻塞，不能用于生产结论。上述能力不代表生产已升级、服务已切换，也不代表 apply-upgrade / restore / rollback、Docker / 1Panel 或 PostgreSQL 已实现。 |

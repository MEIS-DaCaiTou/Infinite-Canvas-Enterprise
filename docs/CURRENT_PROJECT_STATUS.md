...
- `last_verified_code_baseline`：`c5d0a62822bb8b55b25215aa74a9f02653e03fc9`。
...
- ENV-1B2B：资格门禁已选择仍受支持的 ordinary-GIL CPython `3.14.6` Windows x64 / ABI `cp314`；本实现分支迁移唯一 active source policy、30 项应用依赖锁、3 项 bootstrap wheel、builder/verifier 和 Runtime identity/preflight 契约。仓库外 Build A/B、三层 provenance、真实 formal-entry fixture 与最终测试仍作为当前 Draft PR 的动态验收证据；`Python_3_14_candidate_implemented_in_PR=true`、`ENV_1B2B_independent_acceptance_pending=true`、`ENV_1B2_completed=false`、`clean_Windows_validation=false`、`production_approved=false`。实现边界见 [ENV-1B2B 实施记录](./env/ENV-1B2B-PYTHON-314-RUNTIME-IMPLEMENTATION-2026-07.md)。
...
| ENV-1B2B | 当前 Draft 实施阶段 | 将唯一 active Runtime 迁移到 ordinary-GIL CPython 3.14.6 / cp314；独立验收、clean Windows、Manifest v2、activation 和 production approval 均未完成。 |
...
| ENV / 不可变 Release | ENV-1B0、ENV-1B1A、ENV-1B2P、ENV-1B1B、ENV-1B1C-B1/B2 和 ENV-1B2A 已合并。ENV-1B2B 当前 Draft 将唯一 active Runtime 迁移到 CPython 3.14.6 / cp314；独立验收、干净 Windows 验证、完整只读 APP_ROOT、正式不可变 Release、Production Baseline 和 production approval 均未形成。 |

...
ARCH-2A、SEC-1A、SEC-1B1、SEC-1F0 和 SEC-1C0 仓库实现已完成。SEC-1B1、SEC-1F0 与 SEC-1C0 均没有激活 production migration，也没有创建 super_admin；当前下一阶段是 SEC-1B2 受控 migration activation 与首次本机 bootstrap。

每个 P0 安全事项必须使用独立 Issue、独立分支和独立 Draft PR，不将全部 P0 项目打包到一个大 PR。

下一步顺序：

1. SEC-1B2：先加固 SEC-1B1 migration 中剩余未限定 `main.users` 的查询，再在受控维护窗口激活 migration 并实施首次本机 super_admin bootstrap。

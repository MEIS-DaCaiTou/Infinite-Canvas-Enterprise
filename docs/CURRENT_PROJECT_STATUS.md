...
- OPS Release Manifest v2：当前单一 Draft PR 实施 detached manifest、Git-tree payload、闭合 inventory、deterministic archive、Runtime/static/SBOM/licenses/config/database 绑定、offline verifier/materializer 与 portable v2 identity。独立初审提出的 payload-policy authority、minimum contract、SBOM/license closure、config drift 和 path-safety 五项 Blocker 已在同一 PR 的集中 correction candidate 中修正，仍等待新的独立复核；`Manifest_v2_repository_implementation_present=true`、`Manifest_v2_independent_acceptance_pending=true`。activation、OPS-3B、formal Release、clean Windows 和 production approval 仍为 false。实现边界见 [OPS Release Manifest v2 实施记录](./ops/OPS-RELEASE-MANIFEST-V2-IMPLEMENTATION-2026-07.md)。
...
| ENV-1B2B | Repository implementation 已独立验收 | 唯一 active Runtime 已迁移到 ordinary-GIL CPython 3.14.6 / cp314，`ENV_1B2_completed=true`；clean Windows、Manifest v2、activation、formal Release 和 production approval 均未完成。 |
...
| ENV / 不可变 Release | ENV-1B0、ENV-1B1A、ENV-1B2P、ENV-1B1B、ENV-1B1C-B1/B2 和 ENV-1B2A 已合并。ENV-1B2B 的 CPython 3.14.6 / cp314 repository implementation 已独立验收，`ENV_1B2_completed=true`；ENV-1B3、干净 Windows 验证、完整只读 APP_ROOT、正式不可变 Release、Production Baseline 和 production approval 均未形成。 |

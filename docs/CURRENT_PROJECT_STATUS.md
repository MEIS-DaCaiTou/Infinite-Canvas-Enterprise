...
1. ENV-1B0：决策和文档收口；本 PR 只处理文档。
2. ENV-1：完成 APP_ROOT 不可变、路径根、Release-bound Runtime、正式入口、自检、Manifest v2 和干净 Windows 验证。
3. 形成不可变 Windows Release Candidate；Release Candidate 不等于 Production Baseline。
4. DATA-1：建立新生产所需 schema version、migration history、兼容分类和数据完整性基础，不迁移旧生产数据。

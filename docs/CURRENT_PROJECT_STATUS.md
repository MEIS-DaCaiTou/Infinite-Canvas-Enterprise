...
- `last_verified_code_baseline`：`be5573ae416b4ce81f8cc26ae282868a7efa7672`。
- ENV-1B0 / DOC-2 / DOC-2A：PR #80 已合并，merge commit `be5573ae416b4ce81f8cc26ae282868a7efa7672`；完成架构决策冻结、Greenfield 路线和文档事实同步。
- ENV-1B1A：当前 Draft PR 分支实施 APP_ROOT 写入审计、CSS 传递依赖哈希、统一 HTML build ID、确定性 static staging builder 和 `main.py` 最小不可变 Release 兼容补丁；Git tracked 写入 fingerprint/Wxx 漂移门禁随本 PR 验证，在合并前不得描述为已进入 `main`。
...
| ENV / 不可变 Release | ENV-1B0 已合并。ENV-1B1A 在当前 Draft PR 中移除启动/响应阶段 static 自修改，新增 CSS 传递哈希、统一 HTML build ID、显式确定性 staging builder，以及 Git tracked site fingerprint 到 W01-W40 的写入审计漂移门禁；静态分析不证明绝对不存在未知写入。`assets`、`output`、`data`、配置、startup migration、legacy update、bytecode 等仍未迁移，因此完整只读 APP_ROOT、正式不可变 Release 和 Production Baseline 均未形成。 |
...
2. ENV-1B1A：当前 Draft PR 实施 APP_ROOT 写入审计漂移门禁、CSS 传递哈希和统一 HTML build ID；合并前仍不是 `main` 能力，builder 也尚未接入完整 Release / Manifest / activation。
3. ENV-1B2P、ENV-1B1B、ENV-1B1C、ENV-1B2、Manifest v2 和 ENV-1B3：按路线完成 Runtime 证据、路径根、Release-bound 入口、自检和干净 Windows 验证；均未开始。
4. 形成不可变 Windows Release Candidate；Release Candidate 不等于 Production Baseline。
5. DATA-1：建立新生产所需 schema version、migration history、兼容分类和数据完整性基础，不迁移旧生产数据。
6. 独立设计、实现并验证尚不存在的 Fresh Install Bootstrap。
7. 在干净 Windows 环境完成全新安装与初始化验收。
8. 收口 P0 安全、ARCH-3、PERF-1 / OBS-1、浏览器回归和真实 Provider 验收。
9. 使用全新基线数据完成正式 backup 和 restore rehearsal。
10. 完成 OPS-3B 仓库实现；OPS-3B 不用于旧生产，当前尚未开始。
11. 在干净 Windows 环境使用 Fresh Install Bootstrap 建立的全新隔离数据，完成 apply / switch / health / rollback / restore 演练；这是开发或隔离验证，不是生产执行。
12. 由项目负责人确认已经具备经过验证的持续升级和失败恢复能力，并明确批准 Production Baseline。
13. 在生产设备使用全新数据库、账号、配置和凭据进行 Greenfield 部署；新生产尚未部署。
14. 后续正式 Release 才进入新生产版本迭代；OPS-3B 的首次真实生产执行只能发生在 Greenfield 新生产部署以后，并由项目负责人在生产设备本地执行。
15. 完成新生产业务验收后，再由项目负责人单独决定旧生产停止、归档或删除。
16. OPS-3C / Update Center 可在 Production Baseline 后独立实施，不是首次生产部署前置条件。
17. Linux 单服务器后置；PostgreSQL、对象存储、queue、Redis 和多实例按真实需求引入。
...
上游覆盖区 `main.py`、`static/`、`workflows/`、`API/`、`python/`、`VERSION` 默认不修改。只有在受控上游同步、明确 bugfix 或已接受 ADR 要求的最小兼容补丁任务中，才允许最小化修改并在 PR 中说明原因、风险、回滚方案、上游同步保留方法和测试结果。当前 ENV-1B1A 只删除 `main.py` 的 startup/static response 缓存参数运行时生成；未提交任何生成后的 `static` 文件。
...
从 GitHub 最新 `main` 开始新任务；`be5573ae416b4ce81f8cc26ae282868a7efa7672` 是 PR #80 合并后的 ENV-1B1A 起始基线，`396cccc` 和 `a095ce2e` 只作为历史核对基线。main 前进后先读取 `docs/README.md`、本文件、当前 ADR 和最近 PR。

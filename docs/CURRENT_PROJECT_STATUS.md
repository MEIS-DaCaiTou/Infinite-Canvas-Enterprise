...
- 当前稳定 main / origin/main：`68a136a5c71c949b3af9198c1ddfea63ac2c616d`
- 当前上游版本：`2026.07.6`
...
当前进入 OPS-2A：生产运维脚本工具套件第一版，包括 ops-runner、inventory、backup、check-data、validate-release、prepare-upgrade 和 OPS job 本地结构化日志。

由于已有早期版本上线生产，且生产环境与开发 / Codex 环境隔离，3G-8 暂时后置但不取消。Docker / 1Panel / 日志体系纳入 OPS 后续主线。

下一步优先级：

1. OPS-2A：生产运维脚本工具套件第一版。
2. OPS-L1 / OPS-D1：本地结构化日志与 Docker / 1Panel 部署设计，可并行设计。
3. OPS-3：Update Center 页面接入 OPS 能力。
4. OPS-4：生产升级演练。
5. 3G-8：浏览器级自动化回归。
...
确认 HEAD 为 `68a136a5c71c949b3af9198c1ddfea63ac2c616d` 或其后的 main。若 main 已前进，先读取最新 `PROJECT_SCOPE_LOCK.md`、本文件和最近 PR，再开始新任务。

...
| OPS-3A | Draft PR #77 | 可信发布源、严格 manifest、受限下载、Windows 安全 staging、证据绑定的非执行 online-update plan 的仓库实现；只经临时目录和本地 HTTP fixture 验证，不代表生产检查、下载、升级或服务操作已经执行。 |
...
| OPS 工具 | OPS-2A / OPS-2B 已进入 main；OPS-3A Draft PR #77 提供可信 release 查询、下载校验、安全 staging 和非执行 plan。上述能力不代表生产已升级，也不代表 apply-upgrade / restore / rollback、Docker / 1Panel 或 PostgreSQL 已实现。 |

...
| STAB-1 / OPS-L1 | Draft PR #78 | 本地 `3001` upstream / `8000` gateway 独立 supervisor、持久化脱敏日志、原子 runtime state、完整进程 identity、generation-bound lifecycle ACK、优雅 child shutdown 和 Windows Job Object 基础；只经临时 fixture 进程验证，不代表生产服务已切换或 Windows Service 已安装。 |
...
| OPS 工具 | OPS-2A / OPS-2B / OPS-3A 已进入 main；STAB-1 / OPS-L1 Draft PR #78 提供本地 runtime supervisor、持久化日志和 instance-bound lifecycle ACK 基础。上述能力不代表生产已升级、服务已切换，也不代表 apply-upgrade / restore / rollback、Docker / 1Panel 或 PostgreSQL 已实现。 |

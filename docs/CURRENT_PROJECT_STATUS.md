...
- ENV-1B2P：当前 Draft PR 对仓库外既有 Runtime 证据实施标准库、显式路径、无网络的分层验证。真实结果为 core `true`、dependency `false`、archive `false`、`production_approved=false`；合并前不得描述为已进入 `main` 或正式 Runtime 已获批。
...
| ENV-1B2P | 当前 Draft PR | 只读验证既有仓库外证据；core 已验证，dependency rebuild / `pip check` 和 assembled archive 证据不足；固定 `production_approved=false`。 |

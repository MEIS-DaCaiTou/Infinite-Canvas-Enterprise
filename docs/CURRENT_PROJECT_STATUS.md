...
- ENV-1B1B：PR #83 当前 Draft correction pass；PathRoots / `current-release.json` 的 initial implementation 已存在，C2 正在补齐 DB file-level reparse、portable OPS operation-target 和 current-release directory sync 证据；但 `ENV_1B1B_completion_classification=partial`、`ENV_1B1B_acceptance_passed=false`，不实施 Release activation、正式入口/解释器绑定或完整 APP_ROOT 只读。
...
| ENV-1B1B | 当前 Draft PR | PathRoots、严格 current-release pointer、核心数据/配置/upload/ops 路径边界和 workflow overlay；C2 补齐 DB reparse、portable OPS operation-target 与 current-release directory sync 证据；不等于完整只读 APP_ROOT 或 Release activation。 |

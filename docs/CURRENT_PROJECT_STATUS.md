...
- 当前稳定 main / origin/main：`e47389cf3a9454bebc241328772e06d044d52db7`
- 当前上游版本：`2026.07.6`
...
OPS-0 / OPS-0A 已完成，生产环境只读盘点已文档化。OPS-1 已完成，生产备份、离线发布包、升级演练、回滚、数据库迁移与数据治理方案已沉淀。

当前进入 ARCH-1：长期架构蓝图与开发路线确认。ARCH-1 完成后进入 OPS-2A。OPS-2A 目标是生产运维脚本工具套件第一版，包括 ops-runner、inventory、backup、check-data、validate-release、prepare-upgrade 和 OPS job 本地结构化日志。

由于已有早期版本上线生产，且生产环境与开发 / Codex 环境隔离，3G-8 暂时后置但不取消。Docker / 1Panel / 日志体系纳入 OPS 后续主线。

下一步优先级：

1. ARCH-1：长期架构蓝图与开发路线确认。
2. OPS-2A：生产运维脚本工具套件第一版。
3. OPS-L1 / OPS-D1：本地结构化日志与 Docker / 1Panel 部署设计，可并行设计。
4. OPS-3：Update Center 页面接入 OPS 能力。
5. OPS-4：生产升级演练。
6. 3G-8：浏览器级自动化回归。
7. OPS-5 / OPS-6 / OPS-7 / OPS-8：数据完整性、数据治理、SQLite migration、PostgreSQL 迁移 ADR。
8. 3G-6 外部 provider 成功链路补验。
9. 3G-9：生产部署安全治理。
10. 协作权限设计 ADR + 端到端验收矩阵。
11. `enterprise/interceptors.py` 模块化只读审计。
...
确认 HEAD 为 `e47389cf3a9454bebc241328772e06d044d52db7` 或其后的 main。若 main 已前进，先读取最新 `PROJECT_SCOPE_LOCK.md`、本文件和最近 PR，再开始新任务。

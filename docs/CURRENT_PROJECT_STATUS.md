...
- ENV-1B1A：当前 Draft PR 分支实施 APP_ROOT 写入审计、确定性 static staging builder 和 `main.py` 最小不可变 Release 兼容补丁；在合并前不得描述为已进入 `main`。
...
| ENV / 不可变 Release | ENV-1B0 已合并。ENV-1B1A 在当前 Draft PR 中移除启动/响应阶段 static 自修改，新增显式确定性 staging builder 和完整写入审计；`assets`、`output`、`data`、配置、startup migration、legacy update、bytecode 等仍未迁移，因此完整只读 APP_ROOT、正式不可变 Release 和 Production Baseline 均未形成。 |
...
2. ENV-1B1A：当前 Draft PR 实施完整 APP_ROOT 写入审计和 static 构建期哈希；合并前仍不是 `main` 能力。

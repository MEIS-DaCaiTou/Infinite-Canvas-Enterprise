...
部署事实必须单独判断：旧生产仍运行历史版本并已定义为待退役遗留系统；当前仓库继续形成 Production Baseline；未来新生产采用干净环境、全新数据库、全新账号和全新配置进行 Greenfield 部署，但尚未部署。旧生产不作为数据迁移或原地升级来源，也未因该决策停止或删除。
...
当前仍是 JSON / 文件系统 / SQLite 混合存储。owner map 是企业隔离核心，不能绕过。旧生产已有真实用户和业务数据，但这些数据不迁移到未来新生产，也不再进入旧 owner map 修复或旧数据库升级范围；旧生产仍不得未经授权覆盖、清理或删除。未来新生产从全新数据库建立目标 Schema、schema version、migration history 和 mandatory audit，Fresh Install Bootstrap 尚未实现。
...
旧生产现定义为待退役遗留系统，不进行原地升级或数据迁移；本蓝图不授权停止、归档、删除或修改旧生产。

### Greenfield 新生产目标

- 干净或受控清理后的 Windows 环境。
- 不可变 Release 与 Release-bound Runtime。
- 全新数据库、全新账号、全新配置和重新设置的凭据。
- Fresh Install Bootstrap 建立目标 Schema、mandatory audit 和首个 `super_admin`。
- 使用新基线数据完成 backup、restore rehearsal 和升级 / rollback 演练。
- 项目负责人批准 Production Baseline 后才部署；上述能力和尚未部署的新生产均不得写成当前事实。
...
总体顺序保持：先形成可安全初始化、可恢复和可迭代的 Windows Production Baseline，再 Greenfield 部署新生产；随后服务新基线版本演进并推进单机容器化，最后扩大到多服务器与团队协作。
...
| 当前部分具备 | 受控更新入口、操作日志、启动安全警告、旧生产历史只读盘点与备份、生产升级治理设计、OPS-2A runner、OPS-2B Windows wrapper、inventory、backup manifest / copy、check-data、validate-release、prepare-upgrade、OPS job 本地 JSONL、资源引用回溯、外部 provider task owner 拦截。 |
| 当前未实现 | Production Baseline、Greenfield 新生产部署、Fresh Install Bootstrap、Manifest v2、P0 security fixes、apply-upgrade、restore executor、rollback、完整本地 access/app/error/security log、远程日志推送、Dockerfile、docker-compose、1Panel 正式部署、schema migration runner、PostgreSQL、Redis、对象存储生产支持。 |

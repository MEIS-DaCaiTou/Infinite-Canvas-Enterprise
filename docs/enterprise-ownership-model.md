...
## ComfyUI Output Owner

经典画布和 Smart Canvas 的本地 ComfyUI 异步任务使用 `user_canvas_task_map` 记录任务 owner：

- `/api/canvas-image-tasks`
- `/api/canvas-comfy-tasks`

任务详情 GET 成功返回 output 时，企业层按 task owner 补记响应中的本地资源 URL。`/api/view?filename=...&type=output` 和显式 `{filename, subfolder, type: "output"}` 字段都会归一化为 `/assets/output/<subfolder>/<filename>`。这保证普通用户自己的 ComfyUI output 能显示、保存到画布和日志，同时不会把 output 文件名误记为 `/assets/input/*`。

ComfyUI 后端 `missing_node_type`、自定义节点缺失、模型缺失或工作流本身 400 属于本地 ComfyUI 环境 / 工作流问题，不属于企业 owner 模型。

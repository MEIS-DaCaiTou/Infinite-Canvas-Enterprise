...
- 本 PR 新增输出：本审计、`docs/README.md` 和 7 份 ADR
- 删除历史文档：0
...
| 集合 | A | B | C | D | E | F | G | H | 合计 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `main@396cccc...` 审计输入 | 3 | 31 | 6 | 9 | 6 | 2 | 5 | 0 | 62 |
| PR #80 新增输出 | 9 | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 9 |
| 合并审计集合 | 12 | 31 | 6 | 9 | 6 | 2 | 5 | 0 | 71 |

C 和 G 是基线输入时分类，已在本轮同步；E 项保留文件并增加替代关系。9 份新增输出均为当前事实或决策源 A，其中包括新增 ADR-OPS-007。H 为 0，没有因事实无法核验而需要项目负责人裁决的文档。
...
- 项目负责人已接受 ADR-OPS-007：未来生产采用 Greenfield 全新部署，旧生产数据迁移和原地升级计划取消；旧生产仍存在且未停止、归档或删除。
- OPS-3B 仓库实现和使用全新隔离数据的演练是 Production Baseline 前置门禁；首次真实生产执行只发生在 Greenfield 新生产部署后。OPS-3C / Update Center 不是首次生产部署前置条件。
- 新生产、Fresh Install Bootstrap 和 Production Baseline 均尚未实现或部署；`production_device_touched_by_project_owner=true` 是既有项目负责人确认事实，`production_device_touched_by_codex=false`，`production_modified_by_this_PR=false`。
...
| `ARCHITECTURE.md` | 当前仓库运行架构摘要 | C | `a095ce2e` | 本文件 + ADR-ENV-001 / ADR-OPS-007 | 更新 runtime 事实并区分旧生产、仓库基线和未来新生产 |
...
| `docs/architecture/ENTERPRISE-ARCHITECTURE-BLUEPRINT-2026-07.md` | 企业架构蓝图 | C | `a095ce2e` | ADR + 当前状态 | 同步 runtime / OPS 与 Greenfield 生产目标 |
| `docs/bugs/U2-HISTORY-REFRESH-LOSS-INVESTIGATION.md` | 已关闭缺陷调查 | D | PR #62 前后 | 当前状态 | 保留历史 |
| `docs/CURRENT_PROJECT_STATUS.md` | 当前实施事实源 | G | 缺 PR #78/#79 完成事实 | 唯一当前事实源 | 同步 runtime、Greenfield 路线和三层生产状态 |
...
| `docs/ops/OPS-ROADMAP-2026-07.md` | 当前 OPS 路线 | G | 缺 PR #77/#78 合并 | ADR-OPS-006 / ADR-OPS-007 | 同步 ENV 前置，并区分 OPS-3B 基线前仓库实现 / 隔离演练与部署后首次真实生产执行 |
...
| `docs/roadmap/DEVELOPMENT-ROADMAP-2026-2027.md` | 当前总体路线 | C | `a095ce2e` | ENV / OPS ADR | 更新为 ENV-1B 到 Greenfield Production Baseline 路线 |
| `docs/runbooks/SEC-1B2-PRODUCTION-ACTIVATION-RUNBOOK-2026-07.md` | 本机生产人工 runbook | B | SEC-1B2 | ADR-OPS-007 / 当前安全文档 | 保留历史实现语境，不再作为旧生产未来执行入口 |
| `docs/security/SEC-1B1-ROLE-AUTH-VERSION-IMPLEMENTATION-2026-07.md` | SEC-1B1 实施 | B | PR #72 | CURRENT_PROJECT_STATUS | 保留 |
| `docs/security/SEC-1B2-CONTROLLED-ACTIVATION-BOOTSTRAP-2026-07.md` | SEC-1B2 实施 | B | PR #75 | ADR-OPS-007 / runbook | 保留仓库实现；明确不是 Fresh Install Bootstrap，不在旧生产执行 |
...
| `PROJECT_SCOPE_LOCK.md` | 项目范围与禁止边界 | C | U-2 / PR #63 | CURRENT_PROJECT_STATUS、roadmap、ADR-OPS-007 | 同步 ENV-1B 与旧生产非迁移硬边界 |
| `README.md` | 仓库入口 | C | `73a645f` 和旧 launcher 说明 | docs/README、当前状态、ADR-OPS-007 | 更新入口、main 与 Greenfield 生产摘要 |
...
| [ADR-OPS-007](../decisions/ADR-OPS-007-GREENFIELD-PRODUCTION-BASELINE-AND-LEGACY-NON-MIGRATION-2026-07.md) | A | Greenfield Production Baseline 与旧生产非迁移权威决策 |
...
7. 旧生产定义为待退役遗留系统，不再作为迁移或原地升级目标；新生产使用全新数据库、账号、配置和凭据，但尚未部署。
8. SEC-1B2 代码继续保留，但不在旧生产执行，也不等同于尚未实现的 Fresh Install Bootstrap。
9. OPS-3B 仓库实现和隔离环境演练调整为 Production Baseline 前置门禁；首次真实生产执行仍后置于 Greenfield 新生产部署，OPS-3C / Update Center 不作为首次部署前置。
...
| 旧生产迁移 / SEC-1B2 production activation | ADR-OPS-007 明确取消旧生产迁移并禁止在旧生产执行 activation；历史实施文档只保留仓库事实 | 当前决策 |
| 旧生产 `check-data` warning | 保留历史证据，但不再阻塞 Production Baseline，也不是迁移输入 | 当前决策 |
| Fresh Install Bootstrap / 新生产部署 | 当前事实源明确标记尚未实现 / 尚未部署 | 已决策未实施 |
| OPS-3B / OPS-3C 时序 | OPS-3B 仓库实现和隔离演练在 Production Baseline 批准前；首次真实生产执行在 Greenfield 部署后；OPS-3C / Update Center 可后置 | 当前决策 |
| 旧生产停止、归档或删除 | 当前事实源明确否定；未来需要项目负责人单独授权 | 当前边界 |
...
5. ADR-OPS-007 已成为生产路线权威决策；当前事实源和路线图区分旧生产遗留系统、仓库开发基线、Release Candidate、Production Baseline 与未来 Greenfield 新生产。
6. 旧生产盘点与正式备份继续保留为历史已执行事实，但不再作为旧到新迁移输入；DATA-1、未来版本 migration、backup、restore 和 rollback 门禁继续保留。
7. OPS-3B 仓库实现和隔离环境演练是 Production Baseline 前置门禁；其首次真实生产执行只发生在 Greenfield 部署后的正式 Release 迭代，OPS-3C / Update Center 不是首次部署前置。

## 仍需后续实现而非文档决策的问题

- 受支持的新 Python 目标版本由 ENV-1B2 兼容验证决定。
- 在 PR #80 审计时，current-release.json、路径 adapter、static 构建器和正式自检均未实现；当前 ENV-1B1A Draft PR 只实现 static staging builder，前三者中的其它能力仍未实现。
- Manifest v2、DATA-1、Fresh Install Bootstrap、Production Baseline、restore rehearsal、新生产部署和 OPS-3B 尚未实现。

...
2026-09-19 收敛状态：本实施的任务持久回执与阻塞隔离已移植到 `codex/mainline-runtime-convergence-20260919`；现场 `2026.09.4` 增强的全新无 keep-alive 健康连接、并发单飞、2.5 秒外层截止和启动期降级保持也已移植。该分支仍须经 PR/CI 后才可成为 `main`，不能把客户热修现场结果直接外推为主线发布批准。
...
`main.py` 是明确必要的最小兼容接入：startup recovery、两类任务创建/执行/查询改用企业持久回执。核心实现放在 `enterprise/canvas_task_journal.py`，未修改供应商算法或前端。后续修改相关任务入口时必须保留该语义并运行新增测试；本文记录的开发任务本身没有向历史来源仓库发布内容。

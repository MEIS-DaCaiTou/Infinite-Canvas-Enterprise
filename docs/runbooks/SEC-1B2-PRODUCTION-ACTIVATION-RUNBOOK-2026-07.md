...
先按 reviewed plan 的 `session_impact` 确认影响范围：LEGACY 为 `all_existing_sessions`，READY bootstrap-only resume 为 `bootstrap_target_only`。runner 会在读取密码、要求确认短语、预留 report 或打开写数据库前重新执行只读 plan hash、版本、有效期、source fingerprint 和 session impact 验证；失败时停止，不输入密码，不生成 pending report。验证后才交互读取该 selected admin 的当前密码，并要求输入绑定 operation ID、username 和该 session scope 的精确确认短语。没有 password、force、skip、repair、remote 或 break-glass 参数；不要尝试绕过。
...
3. 根据 report 核对 `database_transaction_rolled_back`、`no_database_changes_committed`、`database_changes_committed`、`external_database_changes_detected`、`journal_mode_changed`、`post_commit_verification_required` 与 `do_not_rerun_until_status_verified`。若 journal preparation 报告外部变化，即使 `database_changes_committed=false` 也不得重跑：先排查仍在运行的写入服务、运行 `status`，并在重新开始前创建新的正式 backup。commit 后 verification/report failure 同样不得重试，先运行 `status` 并按 report 和 stderr 确认已提交事实。

...
| 最新 HEAD | `007a240 Merge pull request #38 from MEIS-DaCaiTou/task/3g-4b-asset-library-isolation` |
| 上游基线版本 | `2026.06.23` |
| PR #18-#38 | 已确认合并到 `main` |
...
这只证明项目/画布/资源归属链路已稳定，不代表历史、素材、WebSocket、任务历史、API 设置、工作流设置已经完成隔离。
...
| WebSocket 广播隔离 | 批准 | 处理 `new_image`、任务完成、队列状态、画布更新、素材库更新等事件隔离。 |
| Comfy/video/图片转换任务历史隔离 | 批准但排后 | 涉及异步任务系统，必须在历史、素材、WebSocket 基础隔离稳定后推进。 |
...
- 队列状态。
- 任务完成事件。
- 按当前登录用户、canvas owner、resource owner 进行事件分发。

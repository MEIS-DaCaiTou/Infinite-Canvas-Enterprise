...
UPSTREAM_VERSION_FILE_SHA256 = "07ed50de51e151a77869e9edd7be52d23a7890b0f5f4b9b4b9d75f8d6d90c374"
_VALIDATED_MANIFEST_TOKEN = object()

TOP_LEVEL_KEYS = frozenset({
    "schema_version", "identity", "enterprise_source", "upstream_source",
    "archive", "release_payload", "runtime", "sbom", "licenses",
...
SECTION_KEYS: dict[str, frozenset[str]] = {
    ...
    "licenses": frozenset({"machine_inventory_path", "machine_inventory_sha256", "human_notice_path", "human_notice_sha256", "component_count", "unresolved_count", "legal_review_complete", "inventory_complete"}),
...
def _validate_manifest(payload: object, raw: bytes) -> ReleaseManifestV2:
    ...
    _safe_relative(licenses["machine_inventory_path"]); _safe_relative(licenses["human_notice_path"])
    _sha(licenses["machine_inventory_sha256"]); _sha(licenses["human_notice_sha256"])
    ...
    if type(compatibility["minimum_launcher_contract"]) is not int or type(compatibility["minimum_runtime_contract"]) is not int or compatibility["supported_platform"] != "windows" or compatibility["supported_architecture"] != "x64" or compatibility["portable_release_only"] is not True:
...
def read_release_manifest_v2(path: Path) -> ReleaseManifestV2:
    return parse_release_manifest_v2_bytes(_read_bounded(Path(path), MANIFEST_MAX_BYTES, missing="RELEASE_MANIFEST_MISSING", failed="RELEASE_MANIFEST_READ_FAILED", oversized="RELEASE_MANIFEST_SIZE_INVALID"))
...
def build_inventory(root: Path) -> ReleasePayloadInventory:
    ...
    if not root.is_dir() or root.is_symlink():
        raise ReleaseManifestV2Error("RELEASE_PAYLOAD_ROOT_INVALID")
...
def verify_release_manifest_v2(manifest_path: Path, archive_path: Path, inventory_path: Path, *, expected_enterprise_commit: str | None = None, expected_enterprise_tree: str | None = None) -> ReleaseVerificationResult:
    ...
    license_keys = {"component", "version", "source", "license_expression", "license_text_sha256", "evidence_type", "payload_paths"}
    if license_payload.get("schema_version") != "ops-third-party-license-inventory-v1" or license_payload.get("inventory_complete") is not True or license_payload.get("unresolved_count") != 0 or license_payload.get("legal_review_complete") is not False or type(license_components) is not list or len(license_components) != license_section["component_count"] or any(type(item) is not dict or set(item) != license_keys or not isinstance(item["component"], str) or not item["component"] or not isinstance(item["version"], str) or not isinstance(item["source"], str) or not item["source"] or not isinstance(item["license_expression"], str) or not item["license_expression"] or not _SHA256.fullmatch(str(item["license_text_sha256"])) or not isinstance(item["evidence_type"], str) or not item["evidence_type"] or type(item["payload_paths"]) is not list or not item["payload_paths"] or any(not isinstance(path, str) or path not in by_path for path in item["payload_paths"]) for item in license_components):
    ...
    if static_payload.get("result") != "pass" or static_payload.get("builder_version") != static_section["builder_version"] or static_payload.get("source_tree_digest") != static_section["source_tree_sha256"] or static_payload.get("output_tree_digest") != static_section["output_tree_sha256"] or static_payload.get("html_build_id") != static_section["html_build_id"] or actual_static_digest != static_section["output_tree_sha256"]:
        raise ReleaseManifestV2Error("RELEASE_STATIC_CONTENT_INVALID")
    if app_source_inventory.tree_sha256 != payload_section["app_source_tree_sha256"]:
...
def materialize_release_fixture(manifest_path: Path, archive_path: Path, inventory_path: Path, destination: Path) -> ReleaseVerificationResult:
    ...
    try:
        ...
        if embedded_inventory.read_bytes() != Path(inventory_path).read_bytes():
            raise ReleaseManifestV2Error("RELEASE_MATERIALIZED_INVENTORY_MISMATCH")

"""Read-only recovery assessment and audited clearance for legacy-schema updates.

This module never repairs a database or switches a release.  It only clears a
blocked update after an operator has independently restored a coherent system.
"""
...
from .mvp import (
    LOCK_SCHEMA, MAX_PLAN_BYTES, MAX_RECOVERY_CLEARANCE_BYTES,
    RECOVERY_CLEARANCE_SCHEMA, UpdateJobStore, UpdateMvpError,
    _bounded_json, _canonical, _run_launcher,
)
...
def _database_identity(database_path: Path, app_root: Path, manifest: Any) -> dict[str, object]:
    ...
    try:
        ...
        objects = [
            {"name": row[1], "sql": " ".join((row[3] or "").split()), "table": row[2], "type": row[0]}
            for row in rows
        ]
        if evidence.get("objects") != objects:
            raise UpdateMvpError("SYSTEM_UPDATE_RECOVERY_DATABASE_UNVERIFIED", status_code=409)
        return {"mode": "legacy-exact-schema", "schema_sha256": hashlib.sha256(_canonical(objects)).hexdigest()}
...
def assess_recovery(
    roots: PathRoots, job_id: str, *, database_path: Path,
    ...
    if manifest.release_id != current_id or manifest.raw_sha256 != expected_sha or pointer.release.manifest_sha256 != manifest.raw_sha256:
        raise UpdateMvpError("SYSTEM_UPDATE_RECOVERY_RELEASE_UNVERIFIED", status_code=409)
    verify_materialized_release(app_root, inventory_path=app_root / str(manifest.section("release_payload")["inventory_path"]))
    ...
    identity: dict[str, object] = {
        "job_id": job_id, "job_updated_at": status.get("updated_at"),
        "current_release_id": current_id, "current_manifest_sha256": manifest.raw_sha256,
        "current_pointer_sha256": pointer.raw_sha256, "database": database,
        "health": "healthy", "plan_sha256": hashlib.sha256(_canonical(plan)).hexdigest(),
    }
...
def write_recovery_security_audit(
    ...
    append_security_audit_event(
        action="security.system_update.recovery_clearance", risk_level="L3", result="success",
        actor_type="user", actor_user_id=actor_user_id, actor_role="super_admin",
        operation_id=job_id, capability="system_update", target_type="update_job",
        target_id=job_id, reason=manual_evidence_note,
        context={
...
def clear_recovery(
    roots: PathRoots, job_id: str, *, database_path: Path,
    actor_user_id: str, expected_release_id: str,
    expected_assessment_sha256: str, manual_evidence_note: str,
    ...
    try:
        ...
        report = assess_recovery(roots, job_id, database_path=database_path, launcher=launcher, lock_held=True)
        if report["current_release_id"] != expected_release_id or report["assessment_sha256"] != expected_assessment_sha256:
            raise UpdateMvpError("SYSTEM_UPDATE_RECOVERY_ASSESSMENT_CHANGED", status_code=409)
        record = {
            "schema_version": RECOVERY_CLEARANCE_SCHEMA, "job_id": job_id,
            "actor_user_id": actor_user_id, "verified_at": utc_now(),
            "assessment": report, "manual_evidence_note": manual_evidence_note.strip(),
        }
        ...
        audit(
            ...
            clearance_sha256=record_sha, manual_evidence_note=manual_evidence_note.strip(),
        )
        store.append_event(
            job_id, "RECOVERY_CLEARED", "SYSTEM_UPDATE_RECOVERY_CLEARED",
            actor_user_id=actor_user_id, current_release_id=expected_release_id,

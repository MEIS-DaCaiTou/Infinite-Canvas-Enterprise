...
def test_bridge_release_database_snapshot_matches_customer_095(tmp_path: Path) -> None:
    repository = Path(__file__).resolve().parents[2]
    snapshot = _database_snapshot(repository, tmp_path / "snapshot.tmp")
    payload = json.loads(snapshot)
    assert payload["schema_id"] == "enterprise-database-contract-v1"
    assert "schema_version" not in payload
    assert "versioned_migration_ids" not in payload
    assert "migration_registry_sha256" not in payload
    ...
    assert hashlib.sha256(snapshot).hexdigest() == "6b4a1deca34b5724217db868d1b8015f222982fddf63e952fb73e8d2966900c4"

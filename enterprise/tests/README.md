...
- `test_sec_1b2_activation_bootstrap.py` verifies main-schema migration qualification, caller-owned SEC-1B1/SEC-1F0 transactions, immutable bootstrap lifecycle DDL and exact bootstrap-audit matching, source-bound formal backups, WAL-to-DELETE preparation, plan fingerprints, commit-state reporting, local-only runner surface, full LEGACY-to-ACTIVE rehearsal, dynamic token invalidation, supported resume states, lock rejection, and rollback injection using temporary SQLite databases only.

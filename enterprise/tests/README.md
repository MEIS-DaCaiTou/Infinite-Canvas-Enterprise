...
- `test_env_1b1a_static_build.py` verifies the ENV-1B1A Draft implementation with temporary static trees: full SHA-256 cache parameters, nested HTML and local JS/CSS/image/font references, query/fragment preservation, external/data/blob exclusions, URL-encoded Unicode names, deterministic tree/report output independent of path and mtime, scoped content changes, source immutability, missing/escaping/reparse fail-closed behavior, explicit CLI paths, the real repository static tree staged only under pytest temp, isolated `main.py` import/startup with existing non-static migrations preserved, and repeatable APP_ROOT write-audit coverage. It does not validate a formal Release Runtime or full APP_ROOT immutability.

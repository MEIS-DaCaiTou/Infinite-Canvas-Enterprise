...
- `test_start_stop.ps1` is a manual lifecycle check: it refuses to kill existing listeners, starts the service-host through the fixed runtime CLI, waits for health, requests the owned-only stop command, and confirms `3001/8000` are released.
- `test_stab_1_supervisor_logging.py` verifies STAB-1 with a temporary runtime root, random fixture ports and local HTTP child processes: role-isolated restart, crash-loop blocking, manual stop/no orphan, state atomicity, stream persistence, redaction, rotation, port gate classification, and no-shell/no-browser runtime source boundaries.

...
- `test_sec_1b2_activation_bootstrap.py` verifies main-schema migration qualification, caller-owned SEC-1B1/SEC-1F0 transactions, immutable bootstrap lifecycle DDL, UNINITIALIZED/ACTIVE/RECOVERY_REQUIRED inspection, formal-backup and plan fingerprints, local-only runner surface, full LEGACY-to-ACTIVE rehearsal, token invalidation, supported resume states, lock rejection, and rollback injection using temporary SQLite databases only.

...
- `test_stab_1_supervisor_logging.py` verifies STAB-1 with a temporary runtime root, random fixture ports and local HTTP child processes: role-isolated restart, crash-loop blocking, reserved/adopted lock cleanup, instance-bound ACKs, graceful wrapper shutdown, owned-child fallback after Job termination, state atomicity, stream persistence, explicit-config exact-secret redaction, CLI exit-code contracts, concurrent-stop quiescence, unresolved-listener fail-closed classification, rotation, no-shell/no-browser runtime source boundaries, and the static `enterprise/auth`/`PyJWT` manifest contract. Fixture coverage complements the separate isolated real-application lifecycle smoke; that development-only smoke now covers start, restart, stop, idempotent stop, start-to-stop reuse and independent role recovery. Draft PR #78 remains subject to final review and is not a production conclusion.

...
- `test_stab_1_supervisor_logging.py` verifies STAB-1 with a temporary runtime root, random fixture ports and local HTTP child processes: role-isolated restart, crash-loop blocking, reserved/adopted lock cleanup, instance-bound ACKs, graceful wrapper shutdown, owned-child fallback after Job termination, state atomicity, stream persistence, explicit-config exact-secret redaction, CLI exit-code contracts, concurrent-stop quiescence, unresolved-listener fail-closed classification, rotation, and no-shell/no-browser runtime source boundaries. Fixture coverage cannot replace the separate isolated real-application lifecycle smoke; a restart-to-second-stop ACK / host-exit blocker remains under review in Draft PR #78.

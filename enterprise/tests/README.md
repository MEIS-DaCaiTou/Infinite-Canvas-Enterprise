...
- `test_ops_3a_online_update.py` verifies OPS-3A trusted release metadata, strict `ops-release-manifest-v1`, bounded loopback fixture HTTP with cross-host credential stripping, atomic download, Windows-safe ZIP traversal/ADS/device-name/control-character/duplicate/ZIP-bomb rejection, fresh staging revalidation, source-bound backup and data-check evidence, newer-only release selection, redacted structured jobs, non-executing preparation plans, and direct-runner compatibility using temporary directories only.

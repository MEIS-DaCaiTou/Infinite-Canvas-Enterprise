...
- `test_sec_1c0_super_admin_protection.py` verifies the transitional actor/target role matrix, principal auth-version binding, main.users affected-row and transaction-local readback integrity, denied-audit classification, atomic mandatory audit, last-active-super-admin protection, legacy-mutator rejection, and TEMP-users resistance using temporary SQLite databases only.
